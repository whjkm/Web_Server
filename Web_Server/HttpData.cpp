...
pthread_once_t MimeType::once_control = PTHREAD_ONCE_INIT;             // 一次性初始化
...
{
    pthread_once(&once_control, MimeType::init);        // 一次性初始化，只会执行一次init函数
...
HttpData::HttpData(EventLoop *loop, int connfd):
        ...
        keepAlive_(false)
{
    // Channel 注册 http协议的具体处理方法（注册回调函数）
